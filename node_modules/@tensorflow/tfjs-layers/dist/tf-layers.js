...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function batchFlatten(x) {
        if (ndim(x) <= 1) {
            throw new ValueError("batchFlatten requires a minimum rank of 2. Got rank: " + ndim(x) + ".");
        }
    ...
    function regularNormalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon$$1) {
        if (epsilon$$1 === void 0) { epsilon$$1 = 1e-3; }
        return tfc.tidy(function () {
            var meanAndVariance = tfc.moments(x, reductionAxes);
            var mean$$1 = meanAndVariance.mean;
            var variance$$1 = meanAndVariance.variance;
            var normed = batchNormalization(x, mean$$1, variance$$1, beta, gamma, epsilon$$1);
            return [normed, mean$$1, variance$$1];
        });
    }
    function broadcastNormalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon$$1) {
        if (epsilon$$1 === void 0) { epsilon$$1 = 1e-3; }
        return tfc.tidy(function () {
            var meanAndVariance = tfc.moments(x, reductionAxes);
            var mean$$1 = meanAndVariance.mean;
            var variance$$1 = meanAndVariance.variance;
            var targetShape = [];
            for (var _i = 0, _a = range(0, ndim(x)); _i < _a.length; _i++) {
                var axis = _a[_i];
            ...
            var normed = batchNormalization(x, broadcastMean, broadcastVariance, broadcastBeta, broadcastGamma, epsilon$$1);
            return [normed, mean$$1, variance$$1];
        });
    }
    function normalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon$$1) {
        if (epsilon$$1 === void 0) { epsilon$$1 = 1e-3; }
        if (tfc.util.arraysEqual(reductionAxes.slice().sort(), range(0, ndim(x) - 1))) {
            return regularNormalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon$$1);
        }
        else {
            return broadcastNormalizeBatchInTraining(x, gamma, beta, reductionAxes, epsilon$$1);
        }
    }
    ...
    function dot(x, y) {
        if (ndim(y) !== 2) {
            ...
            if (ndim(x) === 2) {
                ...
                throw new NotImplementedError("dot support for x of rank " + ndim(x) + " is not yet implemented: " +
                    ("x shape = " + shape));
            }
        }
    }
    ...
    function batchNormalization(x, mean$$1, variance$$1, beta, gamma, epsilon$$1) {
        if (epsilon$$1 === void 0) { epsilon$$1 = 1e-3; }
        var out;
        if (ndim(x) === 2) {
            out = tfc.batchNormalization2d(x, mean$$1, variance$$1, epsilon$$1, gamma, beta);
        }
        else if (ndim(x) === 3) {
            out = tfc.batchNormalization3d(x, mean$$1, variance$$1, epsilon$$1, gamma, beta);
        }
        else if (ndim(x) === 4) {
            out = tfc.batchNormalization4d(x, mean$$1, variance$$1, epsilon$$1, gamma, beta);
        }
        else {
            throw new NotImplementedError("batchNormalization is not implememnted for array of rank " + ndim(x) + " " +
                "yet");
        }
        return out;
    }
    function biasAdd(x, bias, dataFormat) {
        return tfc.tidy(function () {
            ...
            if (ndim(bias) !== 1 && ndim(bias) !== ndim(x)) {
                throw new ValueError('Unexpected bias dimensions: ' + ndim(bias) +
    ...
    function binaryCrossentropy(target, output, fromLogits) {
        ...
        return tfc.tidy(function () {
            var y;
            if (!fromLogits) {
                y = tfc.clipByValue(output, epsilon$1(), 1 - epsilon$1());
    ...
    function hardSigmoid(x) {
        return tfc.tidy(function () {
            var y = scalarPlusArray(tfc.scalar(0.5), scalarTimesArray(tfc.scalar(0.2), x));
                ...
                if (spec.ndim != null) {
                    if (ndim$$1 !== spec.ndim) {
                        throw new ValueError("Input " + inputIndex + " is incompatible with layer " + this.name + ": " +
                            ("expected ndim=" + spec.ndim + ", found ndim=" + ndim$$1));
                    }
                }
                if (spec.maxNDim != null) {
                    if (ndim$$1 > spec.maxNDim) {
                        throw new ValueError("Input " + inputIndex + " is incompatible with layer " + this.name +
                            (": expected max_ndim=" + spec.maxNDim + ", found ndim=" + ndim$$1));
                    }
                }
                if (spec.minNDim != null) {
                    if (ndim$$1 < spec.minNDim) {
                        throw new ValueError("Input " + inputIndex + " is incompatible with layer " + this.name +
                            (": expected min_ndim=" + spec.minNDim + ", found ndim=" + ndim$$1 + "."));
                    }
                }
                if (spec.dtype != null) {
    ...
    function categoricalCrossentropy$1(yTrue, yPred) {
        return tfc.tidy(function () { return categoricalCrossentropy(yTrue, yPred); });
    }
    function sparseCategoricalCrossentropy$1(yTrue, yPred) {
        return tfc.tidy(function () { return sparseCategoricalCrossentropy(yTrue, yPred); });
    }
    function binaryCrossentropy$1(yTrue, yPred) {
        return tfc.tidy(function () { return tfc.mean(binaryCrossentropy(yTrue, yPred), -1); });
    }
    ...
    var MAPE$1 = meanAbsolutePercentageError;
    var categoricalCrossentropy$2 = categoricalCrossentropy$1;
    var cosine$1 = cosineProximity;
    var sparseCategoricalCrossentropy$2 = sparseCategoricalCrossentropy$1;
                        ...
                        var _loop_2 = function (metric) {
                            if (['accuracy', 'acc', 'crossentropy', 'ce'].indexOf(metric) !==
                                ...
                                if (outputShape[outputShape.length - 1] === 1 ||
                                    _this.lossFunctions[i] === binaryCrossentropy$1) {
                                    if (['accuracy', 'acc'].indexOf(metric) !== -1) {
                                        accFn = binaryAccuracy;
                                    }
                                    else if (['crossentropy', 'ce'].indexOf(metric) !== -1) {
                                        accFn = binaryCrossentropy$2;
                                    }
                                }
                                    ...
                                    if (['accuracy', 'acc'].indexOf(metric) !== -1) {
                                        ...
                                        accFn = sparseCategoricalCrossentropy$2;
                                    }
                                }
                                else {
                                    if (['accuracy', 'acc'].indexOf(metric) !== -1) {
                                        ...
                                        accFn = categoricalCrossentropy$2;
                                    }
                                }
                                var suffix = void 0;
                                if (['accuracy', 'acc'].indexOf(metric) !== -1) {
    ...
    var Relu6 = (function (_super) {
        ...
        Relu6.prototype.apply = function (x) {
            return tfc.tidy(function () { return tfc.minimum(tfc.scalar(6.0), tfc.relu(x)); });
        };
        ...
        Merge.prototype.call = function (inputs, kwargs) {
            ...
            return tfc.tidy(function () {
                ...
                if (_this.reshapeRequired) {
                    var reshapedInputs = [];
                    var inputDims = inputs.map(function (input) { return ndim(input); });
    ...
    var BatchNormalization = (function (_super) {
        ...
        BatchNormalization.prototype.call = function (inputs, kwargs) {
            ...
            return tfc.tidy(function () {
                ...
                var input = getExactlyOneTensor(inputs);
                var inputShape = shape(input);
                var ndim$$1 = inputShape.length;
                var reductionAxes = range(0, ndim$$1);
                var axis = _this.axis >= 0 ? _this.axis : (_this.axis + ndim$$1);
                reductionAxes.splice(axis, 1);
                var broadcastShape = pyListRepeat(1, ndim$$1);
                broadcastShape[axis] = inputShape[axis];
                var sortedReductionAxes = reductionAxes.slice();
                sortedReductionAxes.sort();
                var needsBroadcasting = !tfc.util.arraysEqual(sortedReductionAxes, range(0, ndim$$1).slice(0, ndim$$1 - 1));
    ...
    function rnn(stepFunction, inputs, initialStates, goBackwards, mask, constants, unroll, inputLength) {
        ...
        var ndim$$1 = inputs.shape.length;
        if (ndim$$1 < 3) {
            throw new ValueError("Input should be at least 3D, but is " + ndim$$1 + "D.");
        }
        var axes = [1, 0].concat(range(2, ndim$$1));
